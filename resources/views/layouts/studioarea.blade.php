    ...
    <nav class="navbar navbar-default menu-navbar">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navcol-1">
                <ul class="nav navbar-nav navbar-right navbar-profile-foto">
                    <li class="dropdown img-profile dropdown-profil">
                        <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false" href="#"><img src="@if(Auth::user()->{'logo studio'}===null)
                    {{ asset('assets/img/user.png') }}
                    @else                    
                    {{ asset('photo')}}/{{ Auth::user()->{'logo studio'} }}
    ...
    <div class="kontent">
        <div class="row heading-edit-akun">
            <div class="col-md-3 col-sm-3 col-profil">
                <div class="col-foto-profile"><img src="@if(Auth::user()->{'logo studio'}===null)
                    {{ asset('assets/img/user.png') }}
                    @else                    
                    {{ asset('photo')}}/{{ Auth::user()->{'logo studio'} }}
                    @endif" class="img-fluid img-profile-cust">
                    <h4>{{ Auth::user()->{'nama studio'} }}</h4><span>{{ Auth::user()->{'email'} }}</span></div>
            </div>
            @yield('bar')
        </div>
        <div class="row row-edit-akun">
            <div class="col-md-3 col-sm-3 col-xs-1 sidenav-profile">
                <ul class="nav nav-tabs nav-samping">
                    @if(Request::is('studio/permintaan')||Request::is('studio/pelaksanaan')||Request::is('studio/selesai'))
                    <li class="profil-btn-nav btn-profil-studio"><a href="{{ route('studio.suntingprofil') }}">Dashboard </a></li>
                    <li class="profil-btn-nav btn-permintaan"><a href="{{ route('studio.permintaan') }}">Permintaan </a></li>
                    @else
                    <li class="profil-btn-nav btn-profil"><a href="{{ route('studio.suntingprofil') }}">Dashboard </a></li>
                    <li class="profil-btn-nav btn-pemesanan"><a href="{{ route('studio.permintaan') }}">Permintaan </a></li>
                    @endif
                </ul>
            </div>
    @yield('content')
